...
# 示例数据假定已经通过某种方式加载到 results_all_configs
results_list <- extract_data(results_all_configs)

# 使用列表中的数据训练机器学习模型
library(randomForest)
# 每个元素的Features需要被转换为数据框或矩阵格式
...
# 训练随机森林模型
rf_model <- randomForest(x = features_matrix, y = scores_vector, ntree = 500)
print(summary(rf_model))

# 使用SVM需要额外的处理，因为SVM在e1071包中通常需要因子输出
library(e1071)
svm_model <- svm(x = features_matrix, y = as.factor(scores_vector), kernel = "radial")
...
ggplot(importance_df, aes(x = reorder(Feature, MeanDecreaseGini), y = MeanDecreaseGini)) +
  ...
  coord_flip() +  # 翻转坐标轴，使其更易读
  labs(title = "Feature Importance", x = "Features", y = "Importance (Mean Decrease in Gini)") +
  theme_minimal()

# 预测
predictions <- predict(rf_model, features_matrix)

# 绘制预测 vs 实际
data_plot <- data.frame(Actual = scores_vector, Predicted = predictions)
ggplot(data_plot, aes(x = Actual, y = Predicted)) +
  geom_point() +
  geom_smooth(method = "lm", col = "red") +
  labs(title = "Actual vs. Predicted Scores", x = "Actual Score", y = "Predicted Score") +
  theme_minimal()
...
# 预测分类结果
predicted_classes <- predict(svm_model, features_matrix)

# 混淆矩阵
confusion_matrix <- confusionMatrix(data = as.factor(predicted_classes), reference = as.factor(scores_vector))

# 打印混淆矩阵
print(confusion_matrix)

# 可视化混淆矩阵
fourfoldplot(confusion_matrix$table, color = c("#6BAED6", "#FD8D3C"), conf.level = 0, margin = 1, main = "Confusion Matrix")

# version 2 of confusion matrix
library(reshape2)  # 用于数据重塑

# 创建混淆矩阵数据框，确保你的数据已经转换为因子类型
confusion_matrix <- table(Predicted = as.factor(predicted_classes), Actual = as.factor(scores_vector))

# 将混淆矩阵转换为长格式
conf_matrix_long <- melt(confusion_matrix)

# 绘制混淆矩阵热图
ggplot(conf_matrix_long, aes(x = Actual, y = Predicted, fill = value)) +
  geom_tile() +  # 用于创建热图中的格子
  geom_text(aes(label = value), vjust = 1.5, color = "white", size = 4) +  # 在格子上添加数值标签
  scale_fill_gradient(low = "blue", high = "red") +  # 定义颜色渐变
  labs(title = "Confusion Matrix", x = "Actual Class", y = "Predicted Class") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # 调整X轴标签的角度以便阅读
...
# 绘制ROC曲线
roc_curve <- roc(response = as.factor(scores_vector), predictor = as.numeric(predictions))
