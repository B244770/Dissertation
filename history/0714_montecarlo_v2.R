...
generate_matrices <- function(nGenos, nEnvs, total_per_env) {
  # allocate genos and reps for each env
  for (env in 1:nEnvs) {
    remaining_plants <- total_per_env
    while (remaining_plants > 0) {
      # randomly choose a genotype
      selected_genotype <- sample(nGenos, 1)
      # restrict the max value of one single allocation
      max_reps <- min(remaining_plants, sample(1:3, 1))
      
      # if chosen, update replication times
...
# multiple runs
nIterations <- 10
results <- vector("list", nIterations)  # save results to a list

for (i in 1:nIterations) {
  result <- generate_and_evaluate(nGenos, nEnvs, total_per_env)
  results[[i]] <- result
}
...
max_score <- max(scores)  # find the max
best_result <- valid_results[[which.max(scores)]]  # get best results

# plot
design_matrix <- best_result$matrices$design_matrix
rep_matrix <- best_result$matrices$rep_matrix

# turn matrix to data frame
design_matrix_df <- as.data.frame(design_matrix)
design_matrix_df$Geno <- rownames(design_matrix_df)
design_matrix_long <- reshape2::melt(design_matrix_df, id.vars = "Geno", variable.name = "Env", value.name = "Planted")

rep_matrix_df <- as.data.frame(rep_matrix)
rep_matrix_df$Geno <- rownames(rep_matrix_df)
rep_matrix_long <- reshape2::melt(rep_matrix_df, id.vars = "Geno", variable.name = "Env", value.name = "Repeats")


# heat map for design matrix
ggplot(design_matrix_long, aes(x = Env, y = Geno, fill = factor(Planted))) +
  geom_tile() +
  scale_fill_manual(values = c("0" = "white", "1" = "blue"), guide = FALSE) +
  scale_y_discrete(breaks = c("Geno1", "Geno250", "Geno500", "Geno750", "Geno999")) +
  theme_minimal() +
  labs(title = "Design Matrix", x = "Environment", y = "Genotype")

# heat map for rep matrix
ggplot(rep_matrix_long, aes(x = Env, y = Geno, fill = Repeats)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "red") +
  scale_y_discrete(breaks = c("Geno1", "Geno250", "Geno500", "Geno750", "Geno999")) +
  theme_minimal() +
  labs(title = "Replicate Matrix", x = "Environment", y = "Genotype")
